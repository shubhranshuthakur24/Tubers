...
def youtubers(request):
    ...
    camera_type_search = Youtuber.objects.all().values_list(
        'camera_type', flat=True).distinct()
    category_search = Youtuber.objects.all().values_list(
...
def search(request):
    ...
    if 'subscriber' in request.GET:
        ...
        if subscriber_count_search:
            tubers = tubers.filter(subs_count__iexact=subscriber)
